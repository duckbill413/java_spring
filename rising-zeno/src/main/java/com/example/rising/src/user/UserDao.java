...
public class UserDao {
    ...
    public Long createUser(PostUserReq postUserReq) {
        String createUserQuery = "insert into user (email, password, phone_number, nickname, profile_img) VALUES (?,?,?,?,?)"; // 실행될 동적 쿼리문
        Object[] createUserParams = new Object[]{postUserReq.getEmail(), postUserReq.getPassword(), postUserReq.getPhoneNumber(), postUserReq.getNickname(), postUserReq.getProfileImg()}; // 동적 쿼리의 ?부분에 주입될 값
    ...
    // 비밀번호 변경
    public int modifyUserPwd(Long userIdx, String newPassword) {
        String modifyUserNameQuery = "update user set password = ? where user_idx = ? "; // 해당 userIdx를 만족하는 User를 해당 nickname으로 변경한다.
        Object[] modifyUserNameParams = new Object[]{newPassword, userIdx}; // 주입될 값들(nickname, userIdx) 순
    ...
    public User getPwd(PostLoginReq postLoginReq) {
        String getPwdQuery = "select user_idx, email, password, phone_number, nickname, profile_img from user where email = ?"; // 해당 email을 만족하는 User의 정보들을 조회한다.
        String getPwdParams = postLoginReq.getEmail(); // 주입될 email값을 클라이언트의 요청에서 주어진 정보를 통해 가져온다.

        return this.jdbcTemplate.queryForObject(getPwdQuery,
                (rs, rowNum) -> User.builder()
                        .userIdx(rs.getLong("user_idx"))
